...
if (legacyPort !== appPort) {
  ...
  proxy.listen(legacyPort, "0.0.0.0", () => {
    console.log(`Legacy Docker port ${legacyPort} forwards to SketchForge PCB on port ${appPort}.`);
  });
}
...
app.on("error", (error) => {
  console.error("Unable to start SketchForge PCB:", error);
  finish(1);
});

app.on("exit", (code, signal) => {
  if (!shuttingDown && signal) {
    console.error(`SketchForge PCB exited from ${signal}.`);
  }
